...
def train_masker(step=1,fresh_fpn=True,visualize=False): # 4 step training
    ...
    for e in range(epoch):
        ...
        for i,img_batch in enumerate(dataloader):
            ...
            gt_labelss=img_batch['labels'] # B x A (all 91 classes, 0-index)
            imgs=imgs.to(device)
...
def test_masker(img):
    ...
    #modules
    mask_rcnn=mo.MaskRCNN(r,pos_threshold,img.shape[1],img.shape[2],fh,fw,mh,mw,device).to(device)
    mask_rcnn.eval()
    ...
    # predict masks
    _,maskss=mask_rcnn(imgs,False,bboxess_f,counts_f) # 1 x K x 1 x ih x iw
